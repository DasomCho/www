// JavaScript Document
 ...
 $(document).ready(function(){
  ...
  $('.tabs .tab0').css('background','url(images/content2/tab0on.jpg)'); // 첫번째 탭메뉴 버튼 on
  ...
  $('.tabs .tab').each(function (index) {     // 각 컨텐츠에 index 번호를 매겨줌
    $(this).click(function(){   //각각의 탭메뉴를 클릭하면, (index:0 1 2 )
	  ...
	  for(var i=0;i<=cnt;i++){ //탭메뉴 개수만큼 loop
           $('.tab'+i).css('background','url(images/content2/tab'+i+'.jpg)'); //모든 탭메뉴 버튼 off
      }
      $(this).css('background','url(images/content2/tab'+(index)+'on.jpg)'); // 클릭한 탭메뉴 버튼 on
   });
  });
});
